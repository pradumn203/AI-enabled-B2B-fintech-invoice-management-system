...
def predict(nulldata):
   ...
   # if :
    ...
    Final1=Final[['doc_id','Aging Bucket','clear_date']].copy()
    doc = Final1["doc_id"].tolist()
    aging= Final1["Aging Bucket"].tolist()
    clearDt=Final1["clear_date"].tolist()
    a=[]
    for i in range(len(doc)):
        a.append({"doc_id":doc[i],"aging_bucket": str(aging[i]),"clear_date": str(clearDt[i])})
